...
from . import browser_setup  # Import the browser setup module
...
from .selenium_utils import ( # Import robust helper functions
    ...
    _wait_for_element_presence, _save_screenshot_on_error, _force_click_js,
...
# Use an absolute path to find config.json relative to this file's location
CONFIG_PATH = os.path.join(os.path.dirname(__file__), 'config.json')

def load_config(config_path=CONFIG_PATH):
    """Load configuration from config.json file using an absolute path."""
    ...
    try:
        ...
        _save_screenshot_on_error(None, logger, "config_not_found")
        exit(1)
    except json.JSONDecodeError as e:
        logger.error(f'Invalid JSON format in {config_path}! {e}')
        _save_screenshot_on_error(None, logger, "config_json_error")
        exit(1)
    except Exception as e:
        logger.error(f'Unexpected error loading config: {str(e)}')
        _save_screenshot_on_error(None, logger, "config_load_error")
        exit(1)
...
def click_okay_button(driver):
    ...
    try:
        ...
        logger.warning("[FORM] Standard click failed on OK button. Attempting forceful JavaScript click.")
        try:
            # If standard click fails, use a more forceful JavaScript click.
            _force_click_js(driver, logger, ELEMENTS["OK_BUTTON_POPUP"])
            logger.info(f'[FORM] Successfully clicked the okay button with JavaScript in {time.time() - start_time:.2f} seconds')
        except Exception as js_e:
            logger.error(f'[FORM] JavaScript click on okay button also failed: {js_e}')
            _save_screenshot_on_error(driver, logger, "click_okay_button_js_failure")
            raise
    except Exception as e:
    ...
    try:
        ...
        logger.warning("[FORM] Timed out waiting for modal backdrop to disappear. This might be okay, proceeding with caution.")
